...
def run(
    ...
    if save_name and (
        ...
        or save_name[-4:] not in (".jpg", ".png", ".bmp", ".gif")
    ):
        raise ValueError(
    ...
    def combine(
        ver, qr_name, bg_name, colorized, contrast, brightness, save_dir, save_name=None
    ):
        ...
        qr_name = (
            os.path.join(
                save_dir, os.path.splitext(os.path.basename(bg_name))[0] + "_qrcode.png"
            )
            if not save_name
    ...
    try:
        ...
        if picture and picture[-4:] == ".gif":
            ...
            qr_name = (
                os.path.join(
                    save_dir,
                    os.path.splitext(os.path.basename(picture))[0] + "_qrcode.gif",
                )
                if not save_name
                else os.path.join(save_dir, save_name)
            )
